...
# La première coordonnée de pos correspond à la ligne,
# et la deuxième à la colonne
# (attention : sur la grille, la position 0.0 est en haut !)
...
# Sert à accéder aux unités à travers un dictionnaire
# pour pouvoir les appeler avec la user input.
...
# Les listes dépendront des tours :
# on associera au nombre de tours modulo 2 pour savoir quelle équipe joue.
...
while end_counter == 0:
    ...
    if nb_turn %2 == 1:
        ...
        ennemy_dico = red_dico
    else:
        ally_unit = red_units
        ally_dico = red_dico
        enemy_unit = blue_units
        ennemy_dico = blue_dico
    ally_name = [x.nb for x in ally_unit]

    # Choix du personnage à bouger
    while ally_unit:  # jusqu'à ce que la liste soit vide
        print("current units :", ally_name)
    ...
    # Choix de l'action à faire
        ...
        while attack_counter == 0:
            if move_counter == 1:
                action = input("possibility : attack, skip unit turn(skip)")
                # place correspond à l'emplacement où l'action va agir
                while action not in ["attack", "skip"]:
                    action = input("fool test")
                if action == "skip":
                    ally_unit.pop(char_name)
            else:
                action = input("possibility : attack, move, skip turn")
                while action not in ["attack", "skip", "move"]:
                    action = input("fool test")
                place = input("choose where to move/attack")
            # Déroulement de l'action
            if action == "move":
                while not possible_action(all_units, place, action):
                    place = input("can't move there, choose again:")
                char.pos = place
            elif action == "attack":
                while not possible_action(enemy_unit, place, action) or possible_action(ally_unit, place, action):
                    ...
                    enemy_dico_pos = {enemy_unit[i].pos: enemy_unit[i] for i in range(len(enemy_unit))}
                    # Permet d'accéder aux unités par leur position
